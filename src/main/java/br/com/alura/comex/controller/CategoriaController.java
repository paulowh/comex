...
public class CategoriaController {
    ...
    public ResponseEntity cadastra(@RequestBody @Valid DadosNovaCategoria form, BindingResult result){
    ...
    public ResponseEntity<Object> buscarPorId(@PathVariable("id") Long categoriaId){
        if (categoriaId == null) return ResponseEntity.badRequest().build();

        Optional<Categoria> categoriaCarregada = categoriaService.buscaPorId(categoriaId);

        if (categoriaCarregada.isPresent()) return ResponseEntity.ok().body(categoriaCarregada.get());
